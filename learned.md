...
### 翻译任务调度失败复盘
- **先验证工具链再派发任务**：工具链没跑通就派发，worker 发现不好用会自己写脚本，浪费时间
- **给 worker 最简单的命令**：一行 `run_translate.bat --dir xxx` 比分步骤 extract→translate→apply→validate 好得多，步骤越多 worker 越容易跑偏
- **共享目录的工作不要并行**：多个任务操作同一个项目目录和工具链会冲突（文件锁、JSON 中间文件覆盖），串行更安全
- **不要为了"看起来快"强行拆并行**：翻译是 IO 密集型（API 调用），拆成多任务并不能加速，反而增加协调成本
- **Worker 会无视复杂指令**：即使明确说"禁止写脚本"，worker 仍可能自己找到 run_all.py 然后跑全量，指令要简单到没有歧义
- **task create --prompt 内容会被截断**：Worker 实际只收到 prompt 的第一句话，详细指令全部丢失。关键操作步骤必须通过 message send 单独发送补充
